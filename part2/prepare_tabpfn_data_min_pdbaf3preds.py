...
# Load datasets
min_5mer = pd.read_csv("RNAcompete/counts/tables_pbdaf3preds_minprobes_tabpfn2.0/5mer_table_tabpfn2.0.csv", sep=',')
max_5mer = pd.read_csv("RNAcompete/counts/tables_pbdaf3preds_minprobes_tabpfn2.0/5mer_table_PDB_AF3_preds_tabpfn2.0.csv", sep=',')

# Combine both dataframes for kmer frequency computation
combined_5mer = pd.concat([min_5mer, max_5mer], ignore_index=True)
combined_5mer = combined_5mer.sample(frac=1, random_state=42).reset_index(drop=True)

# Compute the most frequent RNA 5-mers
top_rna_5mers = combined_5mer["rna_5mer"].value_counts().nlargest(N).index

# Compute the most frequent protein 5-mers
top_prot_5mers = combined_5mer["protein_5mer"].value_counts().nlargest(N).index
...
for df in [min_5mer, max_5mer]:
    for i, group in df.groupby("dataset_id2"):
        sample = {
            ...
            "gc": (group["rna_seq"].values[0].count("G") + group["rna_seq"].values[0].count("C")) / len(
                group["rna_seq"].values[0]),
            "rna_length": len(group["rna_seq"].values[0]),
...
unique_ids = tabpfn_df["uniprot_id"].unique()
random.seed(42)
random.shuffle(unique_ids)

# Select test set IDs
selected_uniprot_ids = set(unique_ids[:test_samples])

# Create test and train sets
test_uniprot = tabpfn_df[tabpfn_df["uniprot_id"].isin(selected_uniprot_ids)]
train_uniprot = tabpfn_df[~tabpfn_df["uniprot_id"].isin(selected_uniprot_ids)]

test_uniprot = test_uniprot.sample(frac=1, random_state=42).reset_index(drop=True)
train_uniprot = train_uniprot.sample(frac=1, random_state=42).reset_index(drop=True)
...
def save_splits(train_df, test_df, split_name, out_dir):
    ...
    train_df[["Id", "prot_seq", "rna_seq", "interaction"]].to_csv(
        f"{out_dir}/{split_name}_{num_features}_features_train_seq.csv", index=False)
    test_df[["Id", "prot_seq", "rna_seq", "interaction"]].to_csv(
        f"{out_dir}/{split_name}_{num_features}_features_test_seq.csv", index=False)

    # Remove k-mer features
    non_kmer_features = ["Id", "prot_seq", "rna_seq", "gc", "rna_length", "interaction"]
    kmer_columns = [col for col in train_df.columns if col not in non_kmer_features]
    train_df.drop(columns=kmer_columns).to_csv(f"{out_dir}/{split_name}_{num_features}_features_train_no_kmer.csv",
                                               index=False)
    test_df.drop(columns=kmer_columns).to_csv(f"{out_dir}/{split_name}_{num_features}_features_test_no_kmer.csv",
                                              index=False)

# Save datasets
out_dir = Path('tabpfn_data/tables_pbdaf3preds_minprobes_tabpfn2.0')
